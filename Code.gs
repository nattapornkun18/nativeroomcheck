...
/* ---- หมวดม่านมีสองทีมเข้ามาแก้คนละเรื่อง จึงมีช่องติดตามงานของแต่ละทีมแยกกัน ----
     🏨 ทีมโรงแรม    ชายม่านลอยพื้นเกินครึ่งนิ้วแต่ไม่ถึง 1 นิ้ว (ปรับตะขอเฉพาะจุด)
                     · แสงลอดเฉพาะจุดที่ไม่ใช่รอยต่อกลางและไม่เกิน 2 จุด
                     · ดึงมือเปิด–ปิดตอนไฟดับต้องได้ 100% · หยอด WD40 ให้รางไม่ฝืด
     🔧 ทีมช่างม่าน  ลอนม่าน ความยาวผ้า ตีนตุ๊กแก ตะขอ หัวสไลด์
                     · มอเตอร์ สวิตช์ ระบบกระตุกกึ่งออโต้ เปิด–ปิดไม่สุด เสียงดัง
                     · แสงลอดที่รอยต่อกลาง หรือลอดหลายจุด
   หน้าเว็บจับทีมให้เองจากข้อที่ไม่ผ่าน สองช่องนี้เป็นแค่สถานะติดตามงาน ไม่ตัดเกรด
   ผลตรวจของห้องยังตัดที่ช่อง "สถานะแก้ไข" เหมือนหมวดอื่น                          */
const TEAM_HOTEL_COL = 'งานทีมโรงแรม', TEAM_CURTAIN_COL = 'งานทีมช่างม่าน';

/* ---- หมวดหลืบห้องน้ำมีสองส่วนที่เดินตรวจคนละรอบ จึงมีรอบตามแก้แยกกันคนละช่อง ----
     🧱 ร่องหลืบขอบผนัง      — กระเบื้องสุดขอบร่องไหม · ในร่องทาสีไหม
     🌀 หลืบพัดลมดูดอากาศ   — สีในหลืบทาลงมาถึงผนังไหม
   สองส่วนนี้ไม่ยุ่งกัน บันทึกส่วนเดียวได้ อีกส่วนที่ยังไม่ได้ตรวจปล่อยว่างไว้
   และช่องรอบตามแก้จะไม่ถูกเติมคำว่า "รอแก้ไข" ให้เอง — ว่าง = ยังไม่ได้ตัดสินใจ
   ช่องรวม "สถานะแก้ไข" หน้าเว็บเติมให้เฉพาะตอนทุกส่วนที่เสียปิดครบแล้วเท่านั้น
   ผลตรวจของห้องยังตัดที่ช่องรวมเหมือนทุกหมวด (ว่าง = ยังไม่ปิดจบ) */
...
const TOPICS = {
  ...
  curtain: {
    ...
    choices: {
      ...
      /* "มือดึง" คือระบบกึ่งออโต้ — กระตุกผ้าแล้วมอเตอร์วิ่งต่อให้เอง เป็นงานทีมช่างม่าน
         คนละเรื่องกับ "ดึงมือไฟดับ" ข้างล่างที่ต้องลากมือล้วน ๆ ได้ 100% (งานทีมโรงแรม) */
      'มือดึงม่านโปร่ง': ['ปกติ', 'กระตุกแล้วไม่วิ่ง'],
      'มือดึงม่านทึบ':   ['ปกติ', 'กระตุกแล้วไม่วิ่ง'],
      /* ทดสอบกรณีไฟดับ: ตัดไฟม่านแล้วลากด้วยมือ ต้องเปิด–ปิดได้เต็มระยะและไม่ฝืด
         ฝืดเมื่อไหร่ทีมโรงแรมหยอด WD40 ที่รางเองได้ ไม่ต้องรอช่างม่าน */
      'ดึงมือไฟดับม่านโปร่ง': ['ดึงมือได้สุด 100%', 'ดึงมือได้ไม่สุด', 'ดึงมือไม่ได้เลย'],
      'ดึงมือไฟดับม่านทึบ':   ['ดึงมือได้สุด 100%', 'ดึงมือได้ไม่สุด', 'ดึงมือไม่ได้เลย'],
      'หยอด WD40': ['หยอดแล้ว', 'รางลื่นอยู่แล้ว ไม่ต้องหยอด', 'ยังไม่หยอด'],
      ...
      /* ปุ่มที่สามไว้ให้ห้องที่ผ้ายังไม่ขึ้น — ไม่มีผืนผ้าก็ส่องหาแสงลอดไม่ได้
         กดแล้วหน้าเว็บซ่อนกลุ่มงานติดตั้งทั้งกลุ่ม หัวข้อพวกนั้นลงชีตเป็น "-" */
      'แสงลอดม่านทึบ':    ['ไม่มีแสงลอด', 'มีแสงลอด', 'ยังไม่ได้ติดตั้งผ้าม่าน'],
      /* ผ้ายังไม่ขึ้น = ยังไม่มีงานให้ตรวจในกลุ่มงานติดตั้ง
         หน้าเว็บไม่ได้ถามข้อนี้แยกแล้ว แต่ยังเติมคอลัมน์นี้ให้จากปุ่มที่กดตรงข้อแสงลอด
         ห้องยังไม่ผ่านและขึ้นเป็นงานทีมช่างม่านจนกว่าจะติดตั้งเสร็จแล้วตรวจซ้ำ */
      'สถานะติดตั้งม่าน':  ['ติดตั้งครบแล้ว', 'ยังติดตั้งไม่เสร็จ'],
      'ตีนตุ๊กแกม่านทึบ':  ['ติดแล้ว', 'ยังไม่ติด'],
      'ตะขอม่านทึบ':      ['ตะขอตะปู', 'ตะขอธรรมดา'],
      'หัวสไลด์ม่านทึบ':   ['สีขาว', 'สีเงิน'],
      /* ชายม่าน: เกินครึ่งนิ้วแต่ไม่ถึง 1 นิ้ว = ปรับตะขอเฉพาะจุด ทีมโรงแรมทำเองได้ (เหลือง)
         1 นิ้วขึ้นไป = ความยาวผ้าผิด ต้องให้ทีมช่างม่านมาแก้ (แดง) */
      ...
      /* สองช่องติดตามงาน ไม่ใช่ผลตรวจ — หน้าเว็บจับทีมให้เองจากข้อที่ไม่ผ่าน
         แล้วผู้ตรวจกดว่าทีมนั้นแก้แล้วยัง ผลตรวจของห้องยังตัดที่ช่องรอบตามแก้เหมือนเดิม */
    ...
    /* แบรนด์เป็นข้อมูลของห้อง ไม่ใช่ข้อบกพร่อง — ทั้งสองค่าถือว่าปกติ ไม่ต้องระบายสี
       ช่องติดตามงานสองทีมก็เหมือนกัน เป็นสถานะงาน ไม่ใช่ข้อบกพร่อง ไม่ต้องระบายสี */
    good: { 'แบรนด์': ['Smart curtain', 'Dooya'],
            /* รางที่ลื่นอยู่แล้วก็ผ่านเหมือนหยอดแล้ว — ค่าปกติจึงมีสองค่า */
            'หยอด WD40': ['หยอดแล้ว', 'รางลื่นอยู่แล้ว ไม่ต้องหยอด'],
            'งานทีมโรงแรม':   ['รอโรงแรมแก้', 'โรงแรมแก้แล้ว'],
            'งานทีมช่างม่าน': ['รอช่างม่าน', 'ช่างม่านแก้แล้ว', 'อนุโลม ไม่ต้องแก้'] },
    warn: ['ดัง', 'มีแสงลอด', 'เกินครึ่งนิ้ว ไม่ถึง 1 นิ้ว', 'ลอนสวยแต่ยับ',
           'ดึงมือได้ไม่สุด', 'ยังไม่หยอด'],
  ...
  bathgap: {
    ...
    /* เกณฑ์รับงาน — ตัวแรกของแต่ละแถวคือค่าที่ผ่าน ที่เหลือเป็นข้อบกพร่อง
       ...
       ส่วนไหนยังไม่ได้เข้าไปตรวจ ช่องนั้นเว้นว่าง = ยังไม่ตัดเกรดส่วนนั้น */
    choices: {
...
const RENAMED = {
  ...
  /* "มือดึง" เดิมหมายถึงกระตุกแล้วมอเตอร์วิ่งให้ ไม่ใช่ดึงมือล้วน ๆ — เปลี่ยนคำให้ตรง
     ตอนนี้แยกเป็นคนละหัวข้อกับ "ดึงมือไฟดับ" ที่เพิ่มเข้ามาแล้ว */
